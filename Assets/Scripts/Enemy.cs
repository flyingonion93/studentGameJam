...
public class Enemy : Characters {
    ...
    public Transform Left, Right, Khame;
    ...
    public void UpdateKing () {
        ...
        if ( ( transform.position.x >= Left.position.x ) && ( transform.position.x <= Left.position.x + 5 ) ) {
            Vector3 aux = new Vector3 ( Right.position.x, GameManager.Instance.Knight.transform.position.y + 10, Right.position.z );
            ...
            Vector3 aux = new Vector3 ( Left.position.x, GameManager.Instance.Knight.transform.position.y + 10, Right.position.z );
            transform.position = Vector3.MoveTowards ( transform.position, aux, walkVel * Time.deltaTime );
            toLeftKing = true;
        } else {
            if ( toLeftKing ) {
